class HabitLogsController < ApplicationController
  before_action :require_habit_plan, only: [:index, :show]

  def index
    # Check if user wants to see all logs or just the week view
    if params[:view] == 'all'
      # Show all logs in table format
      @all_logs = Current.user.active_habit_plan.logs
        .order(date: :desc)
      ...
      @existing_logs = Current.user.active_habit_plan.logs
  ...
  def require_habit_plan
    unless Current.user&.habit_plans.any?
      redirect_to root_path, alert: "Please create a habit plan first to access your habit logs."
      return
    end
  end
end
